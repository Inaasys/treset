...
function obtenultimonumero(){
    $.get(folios_comprobantes_pagos_obtener_ultimo_numero, function(numero){
      $("#numero").val(numero);
    })  
}
//cerrar modales
...
function listar(){
    ...
    tabla=$('#tbllistado').DataTable({
        ...
        serverSide: true,
        ajax: folios_comprobantes_pagos_obtener,
        columns: [
...
//alta
function alta(){
    $("#titulomodal").html('Alta Folio Fiscal Pago');
    ...
    var tabs =  '<div class="row">'+
                    ...
                    '<div class="col-md-3">'+
                        '<label>Serie</label>'+
                        '<input type="text" class="form-control inputnext" name="serie" id="serie" value="P"  onkeyup="tipoLetra(this)" required data-parsley-length="[1, 10]">'+
                    '</div>'+
                    '<div class="col-md-3">'+
                        '<label>Esquema</label>'+
                    ...
                    '<div class="col-md-3">'+
                        '<label>Titulo</label>'+
                        '<input type="text" class="form-control inputnext" name="titulo" id="titulo" value="PAGO" onkeyup="tipoLetra(this)" required data-parsley-length="[1, 20]">'+
                    '</div>'+
                '</div>'+
                '<div class="row">'+
                    '<div class="col-md-12">'+
                        ...
                        '<div class="tab-content">'+
                            '<div role="tabpanel" class="tab-pane fade in active" id="foliostab">'+
                                '<div class="row">'+
                                    '<div class="col-md-6">'+
                                        '<label>Folio Inicial (Los Pagos empiezan con este folio)</label>'+
...
function validararchivostimbrado(){
    ...
    $.ajax({
        ...
        url:folios_comprobantes_pagos_enviar_archivos_timbrado,
        type: "post",
        dataType: "html",
        data: formData,
        cache: false,
...
$("#btnGuardar").on('click', function (e) {
    ...
    if (form.parsley().isValid()){
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            url:folios_comprobantes_pagos_guardar,
            type: "post",
            dataType: "html",
            data: formData,
            cache: false,
...
$("#aceptar").on('click', function(e){
    ...
    if (form.parsley().isValid()){
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            url:folios_comprobantes_pagos_alta_o_baja,
            type: "post",
            dataType: "html",
            data: formData,
            cache: false,
...
function obtenerdatos(numerofolio){
    $("#titulomodal").html('Modificación Folio Fiscal Pago');
    $('.page-loader-wrapper').css('display', 'block');
    $.get(folios_comprobantes_pagos_obtener_folio,{numerofolio:numerofolio },function(data){
    ...
    var tabs =  '<div class="row">'+
                    ...
                    '<div class="col-md-3">'+
                        '<label>Serie</label>'+
                        '<input type="text" class="form-control inputnext" name="serie" id="serie" value="P"  onkeyup="tipoLetra(this)" required readonly data-parsley-length="[1, 10]">'+
                    '</div>'+
                    '<div class="col-md-3">'+
                        '<label>Esquema</label>'+
                        '<select name="esquema" id="esquema" class="form-control select2" style="width:100%" onchange="tipoesquema();" required>'+
                            '<option value="'+data.FolioComprobantePago.Esquema+'" selected>'+data.FolioComprobantePago.Esquema+'</option>'+
                        '</select>'+
                    '</div>'+
                    '<div class="col-md-3">'+
                        '<label>Titulo</label>'+
    ...
    $("#folioinicial").val(data.FolioComprobantePago.FolioInicial);
    if(data.FolioComprobantePago.Esquema != 'CFDI'){
        $("#idactualizarcertificado").attr('onclick','javascript: return false');
    }
    $("#empresa").val(data.FolioComprobantePago.Empresa);
    $("#domicilio").val(data.FolioComprobantePago.Domicilio);
    $("#leyenda1").val(data.FolioComprobantePago.Leyenda1);
    $("#leyenda2").val(data.FolioComprobantePago.Leyenda2);
    $("#leyenda3").val(data.FolioComprobantePago.Leyenda3);
    $("#pagare").val(data.FolioComprobantePago.Leyenda);
    $('.dropify').dropify();
    setTimeout(function(){$("#numero").focus();},500);
    //hacer que los inputs del formulario pasen de una  otro al dar enter en TAB PRINCIPAL
    $(".inputnext").keyup(function (e) {
...
$("#btnGuardarModificacion").on('click', function (e) {
    ...
    if (form.parsley().isValid()){
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            url:folios_comprobantes_pagos_guardar_modificacion,
            type: "post",
            dataType: "html",
            data: formData,
            cache: false,
...
$("#btnpredeterminar").on('click', function(e){
    ...
    if (form.parsley().isValid()){
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            url:folios_comprobantes_pagos_predeterminar,
            type: "post",
            dataType: "html",
            data: formData,
            cache: false,
