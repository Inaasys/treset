  ...
  if(result == false){
    ...
    var fila=   '<tr class="filasproductos" id="filaproducto'+contadorproductos+'">'+
                        ...
                        '<td class="tdmod">'+
                          ...
                          '</select>'+
                        '</td>'+ 
                        '<td class="tdmod"><input type="number" step="0.'+numerocerosconfiguradosinputnumberstep+'" class="form-control divorinputmodsm existenciasalmacenpartida" name="existenciasalmacenpartida[]" id="existenciasalmacenpartida[]" value="0.'+numerocerosconfigurados+'"  onchange="formatocorrectoinputcantidades(this);" readonly></td>'+
  ...
  var tabs ='<div class="col-md-12">'+
              '<div class="row">'+
                '<div class="col-md-3">'+
                  ...
                  '<input type="text" class="form-control inputnextdet" name="folio" id="folio" required readonly>'+
                ...
                '<div class="col-md-3">'+
                  ...
                  '<table class="col-md-12">'+
                    '<tr>'+
                      ...
                      '<td>'+    
                        '<div class="form-line">'+
                          '<input type="text" class="form-control inputnextdet" name="numeropersonalentrega" id="numeropersonalentrega" required data-parsley-type="integer" autocomplete="off">'+
                ...
                '<div class="col-md-4" id="divbuscarcodigoproducto">'+
                  ...
                  '<table class="col-md-12">'+
                    '<tr>'+
                      ...
                      '<td>'+ 
                        '<div class="form-line">'+
                          '<input type="text" class="form-control inputnextdet" name="codigoabuscar" id="codigoabuscar" placeholder="Escribe el código de la herramienta" autocomplete="off">'+
                        '</div>'+
                      '</td>'+
                    '</tr>'+    
                  '</table>'+
                      ...
                      '<table id="tablaherramientasasignadas" class="table table-bordered tablaherramientasasignadas">'+
                        '<thead class="'+background_tables+'">'+
                          '<tr>'+
                            ...
                            '<th class="'+background_tables+'">Existencias Almacén</th>'+
                            '<th class="customercolortheadth">Cantidad</th>'+
                            '<th class="customercolortheadth">Precio $</th>'+
                            '<th class="'+background_tables+'">Total $</th>'+
                            '<th class="customercolortheadth">Estado Herramienta</th>'+
                          '</tr>'+
                        '</thead>'+
                        '<tbody>'+           
                        '</tbody>'+
...
function buscarstringlike(){
  ...
  var tablafolenc=$('#tablafoliosencontrados').DataTable({
      ...
      columns: [
          { data: 'asignacion', name: 'asignacion', orderable: false, searchable: true },
          { data: 'nombre_recibe_herramienta', name: 'nombre_recibe_herramienta', orderable: false, searchable: true },
          { data: 'nombre_entrega_herramienta', name: 'nombre_entrega_herramienta', orderable: false, searchable: true  },
  ...
  $('#tablafoliosencontrados tbody').on('dblclick', 'tr', function () {
      ...
      agregararraypdf(data.asignacion);
  });
}
//generar documento en iframe
function generardocumentoeniframe(asignacion){
  var arraypdf = new Array();
  var folios = [asignacion];
  ...
  $.ajax({
    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
    url:asignacion_herramienta_generar_pdfs,
    data: form_data,
    type: 'POST',
    contentType: false,
    processData: false,
