...
class CotizacionProductoController extends ConfiguracionSistemaController{
    ...
    public function cotizaciones_productos(){
        $serieusuario = 'A';
        $configuraciones_tabla = Helpers::obtenerconfiguraciontabla('CotizacionesProductos', Auth::user()->id);
    ...
    public function cotizaciones_productos_obtener(Request $request){
        if($request->ajax()){
            ...
            $data = VistaCotizacionProducto::select($configuraciones_tabla['campos_consulta'])->where('Periodo', $periodo);
                    ...
                    ->addColumn('operaciones', function($data) use ($tipousuariologueado){
                        $operaciones = '<div class="dropdown">'.
                                            ...
                                            '<ul class="dropdown-menu">'.
                                                ...
                                                '<li><a class="paddingmenuopciones" href="'.route('cotizaciones_productos_generar_pdfs_indiv',$data->Cotizacion).'" target="_blank">Ver Documento PDF - Formato Interno</a></li>'.
                                                ...
                                                '<li><a class="paddingmenuopciones" href="'.route('cotizaciones_productos_generar_pdfs_cliente_indiv',$data->Cotizacion).'" target="_blank">Ver Documento PDF - Formato Cliente</a></li>'.
            ...
            if($rowexcel > 0){
                if (in_array(strtoupper($partida[0]), $arraycodigosyaagregados)) {
                    ...
                    if($contarproductos > 0){
                        ...
                        '<tr class="filasproductos" id="filaproducto'.$contadorproductos.'">'.
                            '<td class="tdmod"><div class="btn btn-danger btn-xs" onclick="eliminarfila('.$contadorproductos.')">X</div><input type="hidden" class="form-control agregadoen" name="agregadoen[]" value="'.$tipooperacion.'" readonly></td>'.
                            '<td class="tdmod"><input type="hidden" class="form-control codigoproductopartida" name="codigoproductopartida[]" value="'.$codigoabuscar.'" readonly data-parsley-length="[1, 20]"><b style="font-size:12px;">'.$codigoabuscar.'</b></td>'.
                            ...
                            '<td class="tdmod"><input type="number" step="0.'.$this->numerocerosconfiguradosinputnumberstep.'" class="form-control inputnextdet divorinputmodsm ivaporcentajepartida" name="ivaporcentajepartida[]" value="'.Helpers::convertirvalorcorrecto(16).'" data-parsley-decimalesconfigurados="/^[0-9]+[.]+[0-9]{'.$this->numerodecimales.'}$/" onchange="formatocorrectoinputcantidades(this);calculartotalesfilas('.$contadorfilas.');"></td>'.
                            ...
                            '<td class="tdmod"><input type="number" step="0.'.$this->numerocerosconfiguradosinputnumberstep.'" class="form-control divorinputmodsm costototalpartida" name="costototalpartida[]" value="'.Helpers::convertirvalorcorrecto(0).'" data-parsley-decimalesconfigurados="/^[0-9]+[.]+[0-9]{'.$this->numerodecimales.'}$/" onchange="formatocorrectoinputcantidades(this);" readonly></td>'.
                            '<td class="tdmod"><input type="number" step="0.'.$this->numerocerosconfiguradosinputnumberstep.'" class="form-control inputnextdet divorinputmodsm comisionporcentajepartida" name="comisionporcentajepartida[]" value="'.Helpers::convertirvalorcorrecto(0).'" data-parsley-decimalesconfigurados="/^[0-9]+[.]+[0-9]{'.$this->numerodecimales.'}$/" onchange="formatocorrectoinputcantidades(this);calculardescuentopesospartida('.$contadorfilas.');" required></td>'.
    ...
    public function cotizaciones_productos_generar_pdfs(Request $request){
        ...
        if($request->imprimirdirectamente == 1){
            $cotizacionesproductos = CotizacionProducto::where('Cotizacion', $request->arraypdf)->get(); 
        }else{
            ...
            if($tipogeneracionpdf == 0){
                ...
                if ($request->has("seriesdisponiblesdocumento")){
                    $cotizacionesproductos = CotizacionProducto::whereBetween('Fecha', [$fechainiciopdf, $fechaterminacionpdf])->whereIn('Serie', $request->seriesdisponiblesdocumento)->orderBy('Folio', 'ASC')->take(1500)->get();
                }else{
    ...
    public function cotizaciones_productos_exportar_excel(Request $request){
        ...
        ini_set('memory_limit', '-1');
        $configuraciones_tabla = Helpers::obtenerconfiguraciontabla('CotizacionesProductos', Auth::user()->id);
        return Excel::download(new CotizacionesProductosExport($configuraciones_tabla['campos_consulta'],$request->periodo), "cotizacionesproductos-".$request->periodo.".xlsx");   
    }
    //configurar tabla
    public function cotizaciones_productos_guardar_configuracion_tabla(Request $request){
        ...
        if($configuraciones_tabla['contar_configuracion_tabla'] > 0){
            Configuracion_Tabla::where('tabla', 'CotizacionesProductos')->where('IdUsuario', Auth::user()->id)
            ->update([
            ...
            $Configuracion_Tabla->tabla='CotizacionesProductos';
